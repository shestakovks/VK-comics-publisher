...
def check_error_in_response(response):
    """Checks if answer is 200 but still contains error.

    Arguments:
        response {dict} -- result of requests.METHOD().json().

    Raises:
        requests.exceptions.HTTPError: Raises if error in answer with 200.
    """
    if 'error' in response:
        raise requests.exceptions.HTTPError(response['error'])


def load_posted(filename='posted.json'):
...
def save_posted(posted, filename='posted.json'):
...
def get_comics(comics_number):
    ...
    comics_json = comics.json()

    comics_pic_url = comics_json['img']
    filename = get_filename_from_url(comics_pic_url)
    download_photo(comics_pic_url, filename)

    comics_comment = comics_json['alt']
...
def get_wall_upload_url(url, params):
    response = requests.get(url.format('photos.getWallUploadServer'),
                            params=params).json()
    check_error_in_response(response)
